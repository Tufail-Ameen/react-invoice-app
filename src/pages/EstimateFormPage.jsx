...
export default function EstimateFormPage() {
  ...
  const {
    ...
    isError,
    error,
  } = useGetEstimateQuery(id, { skip: !isEdit });
  ...
  if (isEdit && (statusUpper === "CONVERTED" || statusUpper === "CANCELLED")) {
    return (
      <div className="page-wrap">
        <EmptyState
          title="Estimate cannot be edited"
          message={`This estimate is ${estimate.status}. Open the detail page instead.`}
        />
        <Link to={`/estimates/${id}`} className="btn edit py-2 px-3">
          View estimate
        </Link>
      </div>
    );
  ...
  const initialValues = {
    clientId:
      estimate?.clientId != null
        ? String(estimate.clientId)
        : estimate?.customerId != null
          ? String(estimate.customerId)
          : "",
  ...
  const saveDraft = async (values) => {
    ...
    try {
      if (isEdit) {
        await updateEstimate({ id, ...payload }).unwrap();
        toast.success("Estimate updated");
        navigate(`/estimates/${id}`);
      } else {
        const result = await createEstimate(payload).unwrap();
